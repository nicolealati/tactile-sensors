...
sensor_data_path = os.path.join(script_dir, 'pressure', finger, 'index.npy')
idle_points_path = os.path.join(script_dir, 'pressure', finger, 'labels', 'idle_points.npy')
rise_points_path = os.path.join(script_dir, 'pressure', finger, 'labels', 'rise_points.npy')
steady_points_path = os.path.join(script_dir, 'pressure', finger, 'labels', 'steady_points.npy')
fall_points_path = os.path.join(script_dir, 'pressure', finger, 'labels', 'fall_points.npy')

sensor_data_path2 = os.path.join(script_dir, 'pressure', finger2, 'index.npy')
idle_points_path2 = os.path.join(script_dir, 'pressure', finger2, 'labels', 'idle_points.npy')
rise_points_path2 = os.path.join(script_dir, 'pressure', finger2, 'labels', 'rise_points.npy')
steady_points_path2 = os.path.join(script_dir, 'pressure', finger2, 'labels', 'steady_points.npy')
fall_points_path2 = os.path.join(script_dir, 'pressure', finger2, 'labels', 'fall_points.npy')
...
sensor_values_filtered2 = np.load(sensor_data_path2)
idle_points2 = np.load(idle_points_path2)
rise_points2 = np.load(rise_points_path2)
steady_points2 = np.load(steady_points_path2)
fall_points2 = np.load(fall_points_path2)

# Map points to labels
signal_length = len(sensor_values_filtered)
signal_length2 = len(sensor_values_filtered2)
...
window_lengths = [2, 2.1, 2.3,  2.5, 2.7, 2.9]  # in seconds
fs=313
overlaps = [0.6, 0.65, 0.7]  # Example overlap values
segments = {wl: {ov: segment_signal(sensor_values_filtered, fs, wl, ov) for ov in overlaps} for wl in window_lengths}
segments2 = {wl: {ov: segment_signal(sensor_values_filtered2, fs, wl, ov) for ov in overlaps} for wl in window_lengths}
...
label_steps2 = np.zeros(len(sensor_values_filtered2))
...
for label_array, label_value in zip(
    [idle_points2, rise_points2, steady_points2, fall_points2], [0, 1, 2, 3]
):
    label_steps2[label_array] = label_value

#plot both label steps
plt.plot(label_steps)
plt.plot(label_steps2)
plt.xlabel("Samples")
plt.ylabel("Class")
...
wavelets = ['db3', 'db4', 'db5', 'sym3', 'sym4', 'sym5', 'coif3', 'coif4', 'coif5']
features = {
...
C_values = [0.1, 1, 10, 20, 50]  # Example range for the regularization parameter
gamma_values = ['scale', 'auto', 0.01, 0.1]  # Example range for the kernel coefficient
