...
class SlidingInterface:
    def __init__(self):
        ...
        #self.force_amp = (np.cos(self.force_x)+1)/2
        #self.applied_force = np.sin(self.force_x)*self.force_amp
    ...
    def LoadPatterns(self): 
        ...
        self.angle_rot = np.array(self.df_pattern['angle_rotation_1'].astype(int))/180*np.pi
    ...
    def SaveData(self):
        ...
        np.save(self.save_dir + "/" + "trigger_clockwise.npy", self.trigger[5])
        np.save(self.save_dir + "/" + "trigger_counterclockwise.npy", self.trigger[6])
        np.save(self.save_dir + "/" + "signal_x.npy", self.x_interp)
        np.save(self.save_dir + "/" + "signal_y.npy", self.y_interp)
        np.save(self.save_dir + "/" + "signal_angle.npy", self.angle_interp)
