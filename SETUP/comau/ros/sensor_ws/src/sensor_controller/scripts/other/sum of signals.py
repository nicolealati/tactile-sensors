...
# i want to sum all the texels of thumb in to a single signla
finger=thumb
finger = np.sum(finger, axis=1)

#detrend lineare del segnale
x = np.arange(len(finger))
z = np.polyfit(x, finger, 2)
p = np.poly1d(z)
new_finger = finger - p(x)
...
detrending_curvedetrending_curve
personale.unbioperson = np.zeros(finger.shape)
