...
class IntegralSensor:
    def __init__(self):
        ...
        rospy.init_node('integral_square_controller')
        self.IntegralService = rospy.Service('/integralsquare', Integral, self.integral)
        self.TareClient = rospy.ServiceProxy('/tare', Tare)
        rospy.wait_for_service('/tare')
        self.Sub = rospy.Subscriber('/integral_sensor', Forcesensor, self.integral_sensor_callback)
        self.Pub = rospy.Publisher('/integral_sensor_square', Forcesensor, queue_size=10)
    ...
    def integral_sensor_callback(self, data):
        if self.start_integral:
            ...
            if self.window_index < self.window_length:
                ...
                self.integral_signal += self.filtered_window[:,:,-1]/self.fs
